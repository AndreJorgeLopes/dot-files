# Generated by Powerlevel10k configuration wizard on 2023-08-11 at 05:50 CEST.
...
()
  ...
  # Determine if the current terminal is warp
  not_warp_terminal() {
  ...
  # Checks if the terminal is warp, and adjusts elements accordingly
  if not_warp_terminal; then
    left_elements+=(
      # =========================[ Line #2 ]=========================
      newline                  # \n
      prompt_char              # prompt symbol
  ...
  # Adds the elements that will be on both lines depending on the terminal
  right_elements+=(
    etf_price   # custom segment that shows the price of ETFs
    battery                    # internal battery
  )
  ...
  fetch_and_update_etf_price() {
    ...
    echo "$(date +%s),$price,$previous_close" > "$HOME/.etf_price_cache"
  }

  # Function to fetch the "08. previous close" value
  get_previous_close_value() {
    local cache_file="$HOME/.etf_price_cache"
    local previous_close=$(cut -d ',' -f3 < "$cache_file")
    echo "$previous_close"
  }


  # Function to get the ETF price from cache or fetch if needed
  get_cached_etf_price() {
    local cache_file="$HOME/.etf_price_cache"
    local current_time=$(date +%s)

    # Check if cache file exists
    if [[ -f "$cache_file" ]]; then
      local last_fetched_timestamp=$(cut -d ',' -f1 < "$cache_file")
    ...
    # Cache expired or doesn't exist, fetch new price asynchronously
    (fetch_and_update_etf_price &)
  }
  ...
  function prompt_etf_price() {
    ...
    p10k segment -i "$icon" -t "%F{${color}}$price%f"
  }

  function instant_prompt_etf_price() {
    prompt_etf_price
  }

  # typeset -g POWERLEVEL9K_ETF_PRICE_ICON_BEFORE_CONTENT=true
  typeset -g POWERLEVEL9K_ETF_PRICE_PREFIX='%fXTNK '
  ...
  # Custom prefix.
  typeset -g POWERLEVEL9K_NVM_PREFIX='%fon '
  # If set to true, the icon will be placed before the content.
  typeset -g POWERLEVEL9K_NVM_ICON_BEFORE_CONTENT=true
  ...
  # Icon before content.
  typeset -g POWERLEVEL9K_PACKAGE_ICON_BEFORE_CONTENT=true
  ...
  # Custon prefix.
  typeset -g POWERLEVEL9K_BATTERY_PREFIX='%fon '
  ...
  # Custom prefix.
  typeset -g POWERLEVEL9K_WIFI_PREFIX='%fusing '
